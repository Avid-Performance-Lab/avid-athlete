...
// ── Programme View ────────────────────────────────────────────────────────────
function ProgrammeView({ athlete }) {
  ...
  if (openSea !== null) {
    ...
    return (
      <SeanceDetail seance={sea} onBack={() => setOpenSea(null)} readOnly />
    )
  }
  ...
  return (
    <div className="fade-in" style={{ padding: '16px 14px' }}>
      ...
      {sem?.seances?.map((sea, i) => {
        ...
        const totalSeries = sea.exercices?.reduce((s, ex) => s + (ex.series?.length || 0), 0) || 0
        return (
          <div key={sea.id} onClick={() => setOpenSea(i)}
            style={{ background: C.card, borderRadius: 10, padding: '14px 16px',
              marginBottom: 10, border: `1px solid ${C.border}`,
              borderLeft: `4px solid ${cc.bg}`, cursor: 'pointer',
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                ...
                <div style={{ fontSize: 10, color: C.muted }}>S{sea.num}</div>
              </div>
              <div style={{ fontSize: 13, color: C.text }}>
                {exCount} exercice{exCount !== 1 ? 's' : ''} · {totalSeries} séries
              </div>
            </div>
